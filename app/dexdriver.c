...
static void send_our_round(void)
{
   ...
   int ok = 0;
   if (ctx->round_idx == 0)
      ok = jpake_round1(ctx->pairing, pkt);
   else if (ctx->round_idx == 1)
      ok = jpake_round2(ctx->pairing, pkt);
   else
      ok = jpake_round3(ctx->pairing, pkt);
   if (!ok) {
...
void driver_on_written(const char *uuid, int status)
{
   ...
   if (ctx->phase == P_SUB1) {
      ...
      if (++ctx->sub_idx < NSUB1) {
         ...
         ctx->did_rounds = 1;
         ctx->pairing    = jpake_new(ctx->g_code, ctx->g_codelen, 1);
      ...
      if (!strcmp(uuid, U_ROUND) && ctx->tx_left > 0) {
         ...
         if (ctx->tx_left == 0) {
            if (++ctx->round_idx < 3) {
               request_round();
            } else if (!jpake_shared_key(ctx->pairing, ctx->shared_key)) {
               LOGI("!! sharedkey fail");
               ctx->phase = P_FAIL;
            } else {
               ctx->have_key = 1;
...
void driver_on_notify(const char *uuid, const uint8_t *buf, int n)
{
   ...
   if (ctx->phase == P_ROUNDS && !strcmp(uuid, U_ROUND)) {
      ...
      if (ctx->rxlen >= 160 && !ctx->round_done) {
         ...
         int ok          = 0;
         if (ctx->round_idx == 0)
            ok = jpake_peer_round1(ctx->pairing, ctx->rxbuf);
         else if (ctx->round_idx == 1)
            ok = jpake_peer_round2(ctx->pairing, ctx->rxbuf);
         else
            ok = jpake_peer_round3(ctx->pairing, ctx->rxbuf);
      ...
      if (n >= 17 && buf[0] == 0x03) {
         ...
         uint8_t expect[8];
         dexcom_dex8(ctx->shared_key, ctx->token, expect);
         ...
         uint8_t reply[9];
         reply[0] = 0x04;
         dexcom_dex8(ctx->shared_key, buf + 9, reply + 1);
