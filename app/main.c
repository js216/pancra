 ...
 * that turns all of that global state into the immutable struct ui.c renders
 * from. The subjects themselves have their own files -- ui.c the rendering,
 * settings.c the persisted state, store.c/stats.c the history and its
 * figures, calib.c the corrections, sensors.c the registry, crashlog.c and
 * tzoff.c the two things a signal handler and the JVM respectively own.
 *
...
/* MA_MARK_PICK (220), not MA_CHAR (300): four other bases sit between them,
 * and the NEAREST one is what bounds this range. Named wrongly, the assert
 * allowed MAX_DEVS up to 100 when the true headroom is 20 -- so raising it
 * would have compiled clean and made a tap on scanned device 20 dispatch
 * MA_MARK_PICK + 0, silently rewriting a sensor's marker instead. */
_Static_assert(MA_DEV_PICK + MAX_DEVS <= MA_MARK_PICK,
               "MA_DEV_PICK range hits MA_MARK_PICK");
 ...
 *
 * NAMED, and g_menu declared with the type, so -Wswitch-enum can see the
 * places that map a screen to something else. As an anonymous enum behind an
 * int it could not, and the three hand-written maps over this set drifted in
 * silence: the three WEIGHT screens were missing from menu_back_code for as
 * long as they had existed, so the back key was dead on all of them and only
 * the X got out (see the comment there). */
enum menu_screen {
   MENU_NONE,
   MENU_SETTINGS,
   MENU_KEYPAD,
   ...
   MENU_SYNCRESTORE, /* confirm pulling the server's record back down */
...
static enum menu_screen g_menu; /* which modal screen is open */
...
/* Why the last keypad entry was refused, shown under the entry field until the
 * next keystroke. Empty means nothing was refused. See ui.h's kp_err. */
static char g_kp_err[40];
...
/* The value the user is presently typing on the rescale keypad, awaiting
 * CONFIRM. Keypad state, not calibration state: calib.c is told the number
 * only once the user commits to it. */
static int g_rescale_entry;
static enum menu_screen g_kp_return; /* where the keypad returns on close */
 ...
 * flag held. Both are mutated from a binder thread -- srec_push memmoves all
 * of g_srec down by one, reachable via ot_drv_done -> sensor_mint. A memmove
 * landing mid-copy renders a garbled or mixed sensor row, and
 * snap_link_for_slot's strcmp against a shifting identity can match the WRONG
 * link, putting one sensor's session age and CONNECTED state on another's row.
 *
...
static void sensor_reconcile(void)
{
    ...
    * srec_push), so this walk needs the registry lock too -- it is the walk
    * that decides g_cur_src, the fallback provenance stamped into the
    * permanent log. */
   sensors_lock();
...
/* Which screen a menu draws as.
 *
 * A TABLE, not a chain of thirty `else if`s ending in a catch-all: the
 * catch-all meant a menu added without its mapping opened and silently
 * rendered the MAIN screen, giving the user a screen they did not ask for
 * and no clue why. Every entry is listed, the switch has no `default:`, and
 * -Wswitch-enum makes the omission a build error instead. */
static int menu_to_scr(enum menu_screen menu)
{
   switch (menu) {
      case MENU_NONE: return SCR_MAIN;
      case MENU_SETTINGS: return SCR_SETTINGS;
      case MENU_KEYPAD: return SCR_KEYPAD;
      case MENU_DEVLIST: return SCR_DEVLIST;
      case MENU_SENSOR: return SCR_SENSOR;
      case MENU_CAL: return SCR_CAL;
      case MENU_CALPEND: return SCR_CALPEND;
      case MENU_RESCALE: return SCR_RESCALE;
      case MENU_RESCALEACT: return SCR_RESCALEACT;
      case MENU_SENSTYPE: return SCR_SENSTYPE;
      case MENU_FORGET: return SCR_FORGET;
      case MENU_LABEL: return SCR_LABEL;
      case MENU_MARKPICK: return SCR_MARKPICK;
      case MENU_COLORPICK: return SCR_COLORPICK;
      case MENU_METERHELP: return SCR_METERHELP;
      case MENU_PAIRCONF: return SCR_PAIRCONF;
      case MENU_SYNCRESTORE: return SCR_SYNCRESTORE;
      case MENU_ADD: return SCR_ADDMENU;
      case MENU_INSULIN: return SCR_INSULIN;
      case MENU_WEIGHT: return SCR_WEIGHT;
      case MENU_WTLOG: return SCR_WTLOG;
      case MENU_WTDEL: return SCR_WTDEL;
      case MENU_INSLOG: return SCR_INSLOG;
      case MENU_DISPLAY: return SCR_DISPLAY;
      case MENU_DEVICES: return SCR_DEVICES;
      case MENU_PERMS: return SCR_PERMS;
      case MENU_OLDDEV: return SCR_OLDDEV;
      case MENU_RECONF: return SCR_RECONF;
      case MENU_REMOTE: return SCR_REMOTE;
      case MENU_INSDEL: return SCR_INSDEL;
      case MENU_ALARM: return SCR_ALARM;
      case MENU_EXPORT: return SCR_EXPORT;
   }
   return SCR_MAIN; /* unreachable: the switch is exhaustive */
}
...
#define UI_PTS_MAX (PLOT_LONG_MAX + NINS + NWT)
   ...
   *m        = (struct screen){0};
   m->scr    = g_gate ? SCR_GATE : menu_to_scr(g_menu);
   m->now    = now;
   m->tz_off = g_tz_off;
   ...
   if (g_menu == MENU_RESCALEACT) {
      ...
      m->rescale_pm = calib_rescale_preview(
          g_rescale_entry, calib_raw_on_link(link_for_slot(g_sel)));
   } else {
      m->rescale_pm = 1000;
   }

   m->kp_mode = g_kp_mode;
   (void)snprintf(m->kp_err, sizeof m->kp_err, "%s", g_kp_err);
...
/* calib.c asks for a repaint when a calibration or rescale resolves. It has
 * no business knowing how the UI schedules one, and this is the whole of what
 * it needs from us. */
...
void pancra_glucose(int mg_dl, int trend, int age_s)
{
   ...
   int rpm             = 1000;
   mg_dl =
       calib_on_reading(src, driver_link(), t, mg_dl, &rpm, &rescale_started);
    ...
    * and the driver is selected to this link, exactly what calib_try_locked
    * needs.
    */
   calib_try_locked(src);
   ...
   if (isnew) {
      /* SAY SO IF IT DID NOT PERSIST. The reading is already in g_hist, so it
       * is on screen and the alarm has seen it -- and after a restart it will
       * be gone. Silence there means the user's record quietly loses samples
       * with no sign until they go looking, which is exactly why the insulin
       * path refuses out loud. */
      if (store_append(t, mg_dl, trend, g_conn_rssi, has, src, t, g_tz_off,
                       KIND_CGM, rpm) != 0)
...
static int wt_action(int action)
{
   if (action == MA_WT_OPEN) {
       ...
       * than converting it. */
      settings_set_wunits((g_wunits == WT_LB) ? WT_KG : WT_LB);
   } else {
      return 0; /* not ours */
   }
   return 1;
...
static int submenu_action(int action)
{
   if (action == MA_ALARM_LOW || action == MA_ALARM_HIGH ||
      ...
      g_menu = MENU_PERMS;
   } else if (action == MA_REMOTE_OPEN || action == MA_SYNCREST_NO) {
      ...
      settings_set_remote_on(!g_remote_on);
   } else if (action == MA_SYNC_EMAIL) {
      g_entrylen    = 0;
      g_kp_err[0]   = 0; /* a freshly opened editor carries no refusal */
      ...
      g_entrylen    = 0;
      g_kp_err[0]   = 0; /* likewise: opening is not a refusal */
      g_label_field = LABEL_SERVER;
      for (int i = 0;
           g_remote_server[i] && g_entrylen < (int)sizeof g_entry - 1; i++) {
 ...
 *
 * The threshold and calibration keypads ACCEPT mmol/L when g_units is set, so
 * a refusal quoting mg/dL sends the user to check a number they never typed --
 * the comment on the calibration branch already notes that mmol/L users are
 * the ones who hit that bound. ui.c does this correctly for the MAX: line it
 * draws one row above; this is the same conversion for the refusal text. */
...
static void menu_action(int action)
{
   ...
   /* --- calibration: user-initiated only, never automatic --- */
      ...
      int pend = (g_sel >= 0 && g_sel < g_nslot &&
                  calib_queued_for(g_slot[g_sel].id) > 0);
      if (pend) {
         g_menu = MENU_CALPEND;
      } else {
      ...
      g_menu = MENU_SENSOR;
              ...
              calib_rescale_engaged(g_slot[g_sel].id)) {
      ...
      if (g_rescale_entry > 0 && g_sel >= 0 && g_sel < g_nslot)
         calib_rescale_set(g_slot[g_sel].id,
                           calib_raw_on_link(link_for_slot(g_sel)),
                           g_rescale_entry);
      g_rescale_entry = 0;
      g_menu          = MENU_SENSOR;
       ...
       * calib_try_locked in pancra_glucose) until the sensor accepts or the
   ...
   /* --- keypad (opened from settings rows: return there on close) --- */
      ...
      g_kp_err[0] = 0; /* a new keystroke retires the last refusal */
      if (g_entrylen < cap)
      ...
      g_kp_err[0] = 0; /* a correction retires the last refusal too */
      if (g_entrylen > 0)
         g_entrylen--;
   } /* backspace */
   else if (action == 113) {
      ...
      if (g_menu == MENU_LABEL && g_label_field == LABEL_EMAIL) {
         ...
         int n =
             g_entrylen < (int)sizeof em - 1 ? g_entrylen : (int)sizeof em - 1;
         int at           = 0;
         int dot_after_at = 0;
         for (int i = 0; i < n; i++) {
            char c = g_entry[i];
         ...
         if (n < 5 || at != 1 || !dot_after_at || em[0] == '@' ||
            ...
            (void)snprintf(g_kp_err, sizeof g_kp_err, "NOT AN EMAIL ADDRESS");
            g_entrylen = 0;
            g_ui_dirty = 1;
            return;
         }
         ...
         if (!remote_server_valid(host)) {
            ...
            (void)snprintf(g_kp_err, sizeof g_kp_err, "NOT A SERVER NAME");
            g_entrylen = 0;
            g_ui_dirty = 1;
            return;
         }
         ...
         if (g_entrylen > 0) {
            ...
            if (mgdl < 0) {
               ...
               (void)snprintf(g_kp_err, sizeof g_kp_err, "RANGE IS %s..%s", lo2,
                              hi2);
               g_entrylen = 0;
               g_ui_dirty = 1;
         ...
         if (g_entrylen > 0) {
            ...
            if (mgdl < 0) {
               ...
               (void)snprintf(g_kp_err, sizeof g_kp_err, "RANGE IS %s..%s", lo3,
                              hi3);
               g_entrylen = 0;
               g_ui_dirty = 1;
         ...
         if (g_entrylen > 0) {
            ...
            if (bad || mgdl > AL_ENTRY_MAX || (!islow && mgdl <= 0) ||
                ...
                *
                * Typing 1500 was answered with "HIGH MUST BE >= LOW", which
                * is not why it was refused and sends the user to change the
                * other number. Say which rule was broken, and say it in the
                * units the keypad is accepting. */
               char bnd[8];
               fmt_bound(bnd, sizeof bnd, AL_ENTRY_MAX);
               if (bad)
                  (void)snprintf(g_kp_err, sizeof g_kp_err,
                                 g_units ? "USE ONE DECIMAL, LIKE 5.5"
                                         : "DIGITS ONLY");
               else if (mgdl > AL_ENTRY_MAX)
                  (void)snprintf(g_kp_err, sizeof g_kp_err, "MAX IS %s", bnd);
               else if (!islow && mgdl <= 0)
                  (void)snprintf(g_kp_err, sizeof g_kp_err,
                                 "HIGH CANNOT BE ZERO");
               else
                  (void)snprintf(g_kp_err, sizeof g_kp_err,
         ...
         if (g_entrylen > 0) {
            ...
            if (bad || ip > 999 || wt_from_tenths(tenths, g_wunits) <= 0) {
               ...
               (void)snprintf(g_kp_err, sizeof g_kp_err, "WEIGHT OUT OF RANGE");
               g_entrylen = 0;
               g_ui_dirty = 1;
         ...
         if (g_entrylen > 0) {
            ...
            if (v < INS_UNITS_MIN || v > INS_UNITS_MAX) {
               ...
               (void)snprintf(g_kp_err, sizeof g_kp_err, "UNITS MUST BE %d..%d",
...
static int menu_back_code(void)
{
   switch (g_menu) {
      ...
      case MENU_SYNCRESTORE: return MA_SYNCREST_NO;
       ...
       * screens above shipped without a back key. MENU_NONE is "no modal is
...
int ot_drv_reading(long naive, int mg_dl)
{
   ...
   if (isnew) { /* meters are never rescaled: factor 1000 */
      if (store_append(t, mg_dl, 127, 0, 0, g_meter_src, naive, tz, KIND_BGM,
                       1000) != 0)
...
void ot_drv_done(int new_records)
{
   ...
   if (g_meter_src && g_meter_model[0] && g_meter_fw[0]) {
      ...
      int cr = sensor_complete(g_meter_src, "", g_meter_model, g_meter_fw, 0);
      if (cr == 1)
...
void pancra_backfill(int mg_dl, int trend, int age_s)
{
   ...
   hist_unlock();
   if (isnew) {
      /* Backfill: no RSSI. A failure here loses a point the sensor already
       * held, so it is worth the same visible refusal. */
      if (store_append(t, mg_dl, trend, 0, 0, src, t, g_tz_off, KIND_CGM,
                       rpm) != 0)
...
static void notify_update(void)
{
   ...
   hist_unlock();
   plot_render((struct plot_fb){g_notify_px, NOTIFY_W, NOTIFY_W, NOTIFY_H},
               (struct plot_rect){0, 0, NOTIFY_W, NOTIFY_H}, pts, np,
               realtime_s(), 3, 3, white_color, -1, 0, g_tz_off);
      ...
      if (AInputEvent_getType(ev) == AINPUT_EVENT_TYPE_MOTION) {
         ...
         if ((begin || cont) && plot_rect(&rx, &ry, &rw, &rh)) {
            ...
            if (begin) {
               int oy70 = ry + rh;
               {
                  ...
                  if (!plot_point_xy((struct plot_rect){rx, ry, rw, rh}, ref,
                                     realtime_s(), g_plot_hours, &oxx, &oy70))
               ...
               for (int i = np_glu; i < np && !have_ins; i++) {
                  ...
                  if (pts[i].hidden ||
                      !plot_point_xy((struct plot_rect){rx, ry, rw, rh}, pts[i],
                                     realtime_s(), g_plot_hours, &oxx, &oyy))
                     continue;
                  int dx = oxx - fx;
                  if (dx < 0)
                     dx = -dx;
            ...
            int idx = plot_hit((struct plot_rect){rx, ry, rw, rh}, pts, np,
                               realtime_s(), g_plot_hours, fx, fy, aim_ins);
            if (idx != g_scrub_idx) {
               g_scrub_idx = idx;
               draw(g_win);
            }
 ...
 * hour out across a DST boundary. Converting needs an instant, and the
 * instant needs the offset, so resolve iteratively: guess with the current
 * offset, ask what the offset was then, and re-derive. Converges in one step
 * except inside the ambiguous repeated hour. */
static long meter_tz_for(long naive)
{
                         ...
                         size_t saved_size)
{
    ...
    * are addresses of objects in other translation units, and g_menu is an
    * enum whose address needs the cast to be read as the int it is. */
   ...
   cctx.where  = &g_where;
   cctx.status = g_status;
   cctx.glu    = &g_cur_glu;
   cctx.menu   = (const volatile int *)&g_menu;
   cctx.nhist  = &g_nhist;
   ...
   if (!g_inited) {
       ...
       */
      {
         ...
         calib_paths(dir); /* cal.q and rescale.cfg */
         ...
         if (lrc < 0) {
            LOGW("store_load: readings log could not be read whole; the "
                 "history shown is INCOMPLETE");
         ...
         settings_load();
         calib_load();      /* queued calibration + active rescale factor */
