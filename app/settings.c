...
void info_save(void)
{
   ...
   if (fd < 0) {
      LOGW("settings: cannot open %s for writing; "
   ...
   n     = clampn(n, sizeof b);
   if (write(fd, b, n) != n) {
      LOGW("settings: sensor info not saved (short write)");
   }
   close(fd);
}
...
void alarm_save(void)
{
   ...
   n     = clampn(n, sizeof b);
   if (write(fd, b, n) != n) {
      LOGW("settings: ALARM THRESHOLDS not saved (short write)");
   }
   close(fd);
}
...
void settings_save(void)
{
   ...
   n = clampn(n, sizeof b);
   if (write(fd, b, n) != n) {
      LOGW("settings: settings not saved (short write)");
   }
   close(fd);
}
...
void code_save(void)
{
   ...
   if (write(fd, g_code_str, n) != n) {
      LOGW("settings: pairing code not saved (short write)");
   }
   close(fd);
}
...
void remote_save(void)
{
   ...
   n     = clampn(n, sizeof b);
   if (write(fd, b, n) != n) {
      LOGW("settings: server settings not saved (short write)");
   }
   close(fd);
}
