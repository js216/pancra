...
static struct mutex ex_stage_lk = MUTEX_INIT;

static int exercise_load_staged(void);

int exercise_load(void)
{
   mutex_lock(&ex_stage_lk);
   int rc = exercise_load_staged();
   mutex_unlock(&ex_stage_lk);
   return rc;
}
