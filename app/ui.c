 ...
 *
 * AND IT STAYS A GLOBAL, deliberately. A review measured that this one counter
 * makes 46 of this file's 100 renderers "effectful" and proposed returning it
 * from the render entry points instead. That trade is a bad one: the increment
 * happens in draw_cell, draw_frame and fill_rect -- the three leaf primitives
 * -- which are reached from all 46, so threading a count out of them means an
 * out-parameter on every drawing primitive and on every caller of one, to move
 * a number no drawing decision ever reads.
 *
...
#define UI_PLOT_MAX (PLOT_LONG_MAX + NINS + NWT)
   ...
   plot_render((struct plot_fb){px, fb->stride, fb->width, fb->height},
               ...
               m->plot_hours, prad, white_color, scrub ? m->scrub : -1,
               UI_HILITE, m->tz_off);
                           ...
                           const struct screen *m, struct hits *h)
{
   ...
   for (int k = 0; k < 2; k++) {
      ...
      if (m->ins_marker[k] == MARK_HIDE) {
         ...
         plot_marker_glyph(
             (struct plot_fb){px, fb->stride, fb->width, fb->height},
             rx - (6 * sc), y + (3 * sc), gr, m->ins_marker[k],
...
/* Exact string equality. ui.c has no <string.h> (the freestanding build's shim
 * does not carry strcmp), and the one caller below needs to tell "SYNCED" from
 * "SYNC FAILED" -- which no prefix test can do. */
static int ui_streq(const char *a, const char *b)
{
   int i = 0;
   for (; a[i] && b[i]; i++)
      if (a[i] != b[i])
                          ...
                          const struct screen *m, struct hits *h)
{
   ...
   const char *em = (m->sync_email && m->sync_email[0]) ? m->sync_email : 0;
    ...
    * -1): it reports, it does not act. */
   {
      ...
      /* Colour by what the field ACTUALLY holds.
       *
       * This used to test rs[0] == '2', a leftover from when the field carried
       * an HTTP status code. sync_report only ever passes SYNCED, SYNC FAILED,
       * NOT PAIRED, PAIRED and PAIRING FAILED -- not one of which begins with
       * '2' -- so the green branch was unreachable and success and failure
       * rendered in identical grey. That is the same defect the comment two
       * lines above describes this row being added to fix. */
      uint32_t rcol = 0xFFAAAAAA; /* "--": nothing has been attempted yet */
      if (ui_streq(rs, "SYNCED") || ui_streq(rs, "PAIRED"))
         rcol = 0xFF33FF88; /* green */
      else if (ui_streq(rs, "SYNC FAILED") || ui_streq(rs, "PAIRING FAILED") ||
               ui_streq(rs, "NOT PAIRED"))
         rcol = 0xFFFF5555; /* red, as the keypad refusals use */
      menu_row(fb, h, y, sc, lh, "LAST STATUS", rs, rcol, -1);
      y += 2 * lh;
   }
...
static void render_syncrestore(struct ANativeWindow_Buffer *fb,
                               const struct screen *m, struct hits *h)
{
   ...
   int x        = 4 * sc;
   int y        = (fb->height / 20) + (8 * sc);
   add_hit(h, 0, y - (3 * sc), fb->width, 2 * lh, ACT_MENU, MA_SYNCREST_NO);
   ...
   int bw = fb->width - (2 * x);
   y      = menu_button(fb, h, x, y, bw, sc, "NO", 0xFFFFFFFF, MA_SYNCREST_NO);
   y += 3 * lh;
   menu_button(fb, h, x, y, bw, sc, "RESTORE", 0xFF00FF00, MA_SYNCREST_YES);
}
...
/* MA_CHAR is 300 + an index into this array, and MA_WTTAB begins at 350. The
 * enum's own comment records a tap on the weight plot's "1M" tab once
 * dispatching a letter into the rename keypad, so the distance between them
 * is load-bearing -- and the prose stating it had already drifted (it says 38
 * for what was a 40-character array). This is the same fact as an assertion,
 * which cannot drift. */
_Static_assert(MA_CHAR + (int)(sizeof ui_label_chars) - 1 <= MA_WTTAB,
               "ui_label_chars has grown into MA_WTTAB's action range");
                          ...
                          const struct screen *m, struct hits *h)
{
   ...
   if (m->kp_err[0]) {
      draw_str(px, fb, x, y, sc, m->kp_err, 0xFFFF5555);
      y += 2 * lh;
   }
    ...
    * fat-finger from DEL). */
   int dotkey = (kp_thresh(m->kp_mode) && m->units) || m->kp_mode == 14;
