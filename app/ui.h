...
enum ui_menu {
   ...
   /* 131..133, NOT 37..39: MA_SCTOGGLE below reserves 37..47 for the ADD
    * menu's PIN checkboxes, and putting these there made the FAST INSULIN
    * square open the email editor and SLOW INSULIN open the pairing code.
    * The assert at the end of this enum now makes that collision a build
    * error rather than a surprise on the phone. */
   MA_SYNC_EMAIL  = 131, /* remote menu: edit the account email (text) */
   MA_SYNC_PAIR   = 132, /* remote menu: enter the 6-digit pairing code */
   MA_SYNC_UNPAIR = 133, /* remote menu: forget the paired identity */
   /* remote menu: pull back everything the server holds and this phone does
    * not. Manual by design -- see sync_restore in app/sync.h. */
   MA_SYNC_RESTORE = 134,
   MA_SYNCREST_YES = 135, /* the confirmation: really pull */
   MA_SYNCREST_NO  = 136, /* the confirmation: back out */
...
/* MA_SCTOGGLE is a BASE: the ADD menu adds a slot index to it, so 37..47 all
 * belong to it. Anything landing in that span is silently stolen by whichever
 * comparison runs first -- which is how the insulin squares started opening
 * the sync screens.
 *
 * One comparison, not "a < BASE || a > BASE + 10": that shape reads as a
 * template argument list to anything parsing C++, and the formatter duly
 * reflowed it into `MA_SYNC_PAIR<MA_SCTOGGLE || MA_SYNC_PAIR> MA_SCTOGGLE +
 * 10`. These codes belong ABOVE the reserved span, so saying only that is
 * both simpler and the stronger claim. */
_Static_assert(MA_SYNC_EMAIL > MA_SCTOGGLE + 10,
               "MA_SYNC_EMAIL lands in the MA_SCTOGGLE slot range");
_Static_assert(MA_SYNC_PAIR > MA_SCTOGGLE + 10,
               "MA_SYNC_PAIR lands in the MA_SCTOGGLE slot range");
_Static_assert(MA_SYNC_UNPAIR > MA_SCTOGGLE + 10,
               "MA_SYNC_UNPAIR lands in the MA_SCTOGGLE slot range");
