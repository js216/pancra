 ...
 * by exact name.
 *
 * WHY IT DOES NOT SERVE ONE FIXED NAME, which would be two bugs in one line.
 * An allowed set of one name -- files/pancra.csv -- rewritten on every export
 * makes the provider resolve a LIVE pathname: the URI a recipient was granted
 * names a file whose contents change under it, and a second export truncates
 * the first recipient's data mid-read. Snapshots have unique names, and this
 * side has one job -- resolve a name that is genuinely a snapshot's, and
 ...
 * (ExportPolicy.exportNameValid, host-tested), and the resolved path must
 ...
 * is what a recipient reading during an export would otherwise get.
 *
...
public class PancraFiles extends ContentProvider {
    ...
    private File resolve(Uri uri) throws FileNotFoundException {
        ...
        File dir = new File(c.getFilesDir(), ExportPolicy.EXPORT_DIR);
