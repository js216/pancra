...
static struct mutex ins_stage_lk = MUTEX_INIT;

static int insulin_load_staged(void);

int insulin_load(void)
{
   mutex_lock(&ins_stage_lk);
   int rc = insulin_load_staged();
   mutex_unlock(&ins_stage_lk);
   return rc;
}
