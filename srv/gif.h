 ...
 *
 * WHAT A BIGGER ONE DOES TO AN UNCHECKED ENCODER. Taking any positive `int`
 * and serialising `w & 0xFF` then `w >> 8` -- the low sixteen bits -- into
 * both descriptors, then walking w*h pixels anyway, measured on this
 * machine:
 *
 ...
 * holding a quarter of a million of them. A decoder reads the four it is
 * promised and stops; everything after is trailing rubbish where the block
 * terminator and the 0x3B trailer belong. The file is corrupt, and
...
PANCRA_MUST_USE int gif_dims_ok(int w, int h, size_t *npx);
 ...
 * signature goes down -- a property only visible by filling `out` with a
 * sentinel first, because an argument check placed after the header is written
 * passes anything that looks only at the return value. */
PANCRA_MUST_USE size_t gif_encode(struct gif_ws *ws, uint8_t *out, size_t cap,
