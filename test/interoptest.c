                ...
                const char *body, int blen, char *out, int outcap)
{
   ...
   int blen2 = (int)(got - (size_t)(b - buf));
   if (blen2 > outcap - 1)
      blen2 = outcap - 1;
   if (blen2 > 0)
      memcpy(out, b, (size_t)blen2);
   out[blen2 < 0 ? 0 : blen2] = 0;
...
int main(int argc, char **argv)
{
   if (argc < 5) {
      fprintf(stderr,
              "usage: interoptest <port> <email> <code> <dir>\n");
      return 2;
   }
   g_port           = atoi(argv[1]);
   ...
   char readings[512], insulin[512], weight[512];
