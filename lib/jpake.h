 ...
 * reasoning (including what an accepted-but-unproven round is worth to an
 * attacker) is at the top of struct jpake in jpake.c.
 *
 ...
 *
 *   - passlen > JPAKE_PASS_MAX. NOT clipped to the first 32 bytes
 *     (`if (passlen > 32) passlen = 32;` before a memcpy of passlen bytes):
 *     that makes a 33-byte password and its 32-byte PREFIX the same pairing
 *     secret with no way for either side to notice -- two users who chose
 *     different passphrases sharing a key, reported to nobody. A password
 *     this
 *     construction cannot represent is a caller bug, and pairing on a
 *     different secret is the worst available answer to it.
 *   - passlen == 0, or pass == NULL with passlen != 0. The first produces
 *     the all-zero scalar below; the second is a memcpy from NULL.
 ...
 * once, and the object has to say so. With three independent flags recording
 * which rounds were accepted and each entry point testing whichever subset it
 * happens to need, the object accepts transcripts the protocol has no meaning
 * for: a second round-1 packet OVERWRITES the first one's ephemeral public
 * key -- which is a base point of round 3 and a term in the derived key --
 * and a round-3 packet handed in before round 2 is decoded and verified
 * against a base nobody has proven anything about.
 *
 * So the phase is a COUNTER rather than a set of flags. It only ever counts
 * up, one round at a time, and every entry point names the single phase it
 * belongs to, so a
 ...
 * every required round, and jpake_shared_key demands precisely that rather than
