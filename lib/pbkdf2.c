 ...
 *
 * See pbkdf2.h for each bound's source in RFC 8018 and for what this file
 * used to substitute instead of refusing.
 */
#include "pbkdf2.h"
#include "hmac.h"
#include <string.h>
                                 ...
                                 unsigned iters, uint8_t *out, size_t n)
{
    ...
    * this the platform's limit instead of RFC 8018's. */
   if (n == 0 ||
   ...
   while (n) {
      ...
      for (unsigned i = 1; i < iters; i++) {
         hmac_sha256(pw, pwn, u, PBKDF2_HASH_LEN, u);
